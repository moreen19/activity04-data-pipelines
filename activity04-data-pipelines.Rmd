...
- When the message option is turned on (`TRUE`) the output contains a lot of details on what packages in tidyverse have been attached or loaded but when its turned off (`FALSE`) it doesn't.
...
**Response**:
when turned on the chuck gives more details on spec etc but when turned off it doesn't.
...
- The formula for calculating unemployment rate = unemployed/(unemployed+employed)
- So the variables i would need are, number of those unemployed and employed graduates within each major.Then i would go ahead and use the formula provided to calculate the unemployment rate for each major
- Finally i'll sort the unemployment rate column from lowest to highest...assuming im using excel. 
...
rearrange_college <- arrange(college_recent_grads, unemployment_rate) % > %
lowest_unemploy <- select(college_recent_grads, rank, major, unemployment_rate)
...
There are seven stem majors that have salaries that are below the median salary for all majors.
