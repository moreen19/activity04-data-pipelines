...
-   When the message option is turned on (`TRUE`) the output contains a
    lot of details on what packages in tidyverse have been attached or
    loaded but when its turned off (`FALSE`) it doesn’t.
...
**Response**: when turned on the chuck gives more details on spec etc
but when turned off it doesn’t.
...
-   So the variables i would need are, number of those unemployed and
    employed graduates within each major.Then i would go ahead and use
    the formula provided to calculate the unemployment rate for each
    major
-   Finally i’ll sort the unemployment rate column from lowest to
    highest…assuming im using excel.
...
rearrange\_college &lt;- arrange(college\_recent\_grads,
unemployment\_rate) % &gt; % lowest\_unemploy &lt;-
select(college\_recent\_grads, rank, major, unemployment\_rate)
